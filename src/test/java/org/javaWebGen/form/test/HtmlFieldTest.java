...
import org.javaWebGen.form.HtmlTimeField;
...
public class HtmlFieldTest {
	...
	public void testTimeField(){
		HtmlTimeField field= new HtmlTimeField("testTime",true);
		field.setValue("  "); 
		Assert.assertNotNull("error is?"+field.getErrorMessage(),field.getErrorMessage() );
	 	Assert.assertFalse("''  is not a time"+field.getValue(),field.getValue().length()<1 );

	  
	 	field.setValue(null); 
	 	Assert.assertFalse("null required time?"+field.getValue()+field.isValid(),field.isValid() ); 
	 	//Assert.assertTrue("field is emptyString?"+field.getValue()+"|",field.getValue().length()<1 );
	 	
	 	field.setValue(" "); 
		Assert.assertFalse("is valid time ?"+field.getValue()+field.isValid(),field.isValid() );
		
		field.setValue("8:30 PM");

		Assert.assertTrue("is valid time?"+field.getValue(),field.isValid() );

			
		field.setValue("18:30");
		Assert.assertTrue("is valid time?"+field.getValue(),field.isValid() );
		
		field.setValue("01:30");
		Assert.assertTrue("is valid time?"+field.getValue(),field.isValid() );
